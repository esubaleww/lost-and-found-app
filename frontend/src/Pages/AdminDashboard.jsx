...
const AdminPanel = () => {
  ...
  const updateFoundStatus = (id, status) => {
    fetch(`https://server-production-82bb.up.railway.app/api/items/found_items/${id}/status`, {
      method: 'PUT',
  ...
  const updateLostStatus = (id, status) => {
    fetch(`https://server-production-82bb.up.railway.app/api/items/lost_items/${id}/status`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ status }),
    }).then(loadItems);
  ...
  const deleteItem = (type, id) => {
    const endpoint = `https://server-production-82bb.up.railway.app/api/items/${type}_items/${id}`;
      ...
      <div className="flex flex-row gap-4 mb-12">
        ...
        <div className="flex-1">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-h-[500px] overflow-y-auto">
            {foundItems.map(item => (
              <div key={item.id} className="bg-gray-400 p-4 rounded-lg shadow border flex gap-4 relative">
                <img
                ...
                <div className="flex-1">
                  ...
                  <p className="text-sm text-gray-900">Location: {item.location}</p>
                  <p className="text-sm text-gray-900">Date Found: {new Date(item.date_found).toLocaleDateString()}</p>
            ...
            {lostItems.map(item => (
              <div key={item.id} className="bg-gray-400 p-4 rounded-lg shadow border flex gap-4 relative">
                ...
                <div className="flex-1">
                  <h3 className="font-bold text-lg text-gray-950">{item.name}</h3>
                  <p className="text-sm text-gray-900">Location: {item.location}</p>
                  <p className="text-sm text-gray-900">Date Lost: {new Date(item.date_lost).toLocaleDateString()}</p>
                  <p className="mt-2 text-sm">
                    Status: <span className={statusStyle(item.status)}>{item.status}</span>
                  </p>
                  <select
